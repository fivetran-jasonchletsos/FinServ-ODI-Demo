...
const ENGINES: QueryEngine[] = [
  {
    name: 'Snowflake',
    status: 'active',
    description: 'Primary engine for the Altavest gold layer. Reads Iceberg externals through Polaris catalog; auto-suspends between queries. Where the research portal, the cost-estimator, and the dbt-wizard run-time agents all land. Humans and agents read the same gold layer.',
    sample_query: `SELECT
  c.ticker, c.sector,
  ...
  return (
    <section className="mb-8 research-card overflow-hidden" style={cardStyle}>
      <header className="research-card-header flex items-start justify-between gap-4" style={cardHeaderStyle}>
        <div>
          ...
          <p className="text-sm text-[var(--ink-muted)] mt-1 max-w-3xl">
            ...
            downstream build. Regulatory feeds (EDGAR, FRED, CFPB) sit idle for most of the day;
            Cap IQ reference data changes a few times an hour during market hours. The no-op
            detection sleeps through the quiet stretches and wakes up only when filings drop or
            tickers move.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 divide-y-0 md:divide-x divide-[var(--hairline-soft,#e8e4d8)]">
        ...
        <RecoveryTile label="Annual savings · stack-wide"    big="$20.1k"                                      sub="Zero-row dbt builds + downstream skip · sized to typical Tier-1 buy-side connector footprint" color="#16a34a" />
  ...
  return (
    <section className="mb-8 research-card overflow-hidden" style={cardStyle}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-[var(--hairline-soft,#e8e4d8)]">
        ...
        <CostTile label="Zero-row dbt · saved" value="$2.87"  sub="84% of Fivetran syncs no-op today · downstream dbt builds finish in zero rows" color="#7c3aed" />
...
const GX_SUITES: GxSuite[] = [
  ...
  },
  {
    ...
    passing: 14,
    last_run: '07:14:29',
    why: 'form_type ∈ SEC published list; filing_date ≤ today; accession_number matches NNNNNNNNNN-NN-NNNNNN format (18 digits, two dashes).',
  },
  {
  ...
  },
  {
    ...
    passing: 11,
    last_run: '07:11:09',
    why: 'observation_value not null on published dates; observation_date within last 60 years; one warn on 12 stale series IDs FRED deprecated this month.',
  },
  {
  ...
  },
  {
    ...
    passing: 13,
    last_run: '07:22:51',
    why: '|delta_shares| ≤ 5× prior position flags for human review (catches ingestion glitches that look like 100× changes; corporate-action splits + new activist positions go through a different gate); quarter-over-quarter consistency.',
  },
];
  ...
  return (
    <section className="mb-8 research-card overflow-hidden" style={cardStyle}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 divide-y-0 md:divide-x divide-[var(--hairline-soft,#e8e4d8)]">
        ...
        <RecoveryTile label="Failed-expectation queue"  big="12 rows"                             sub="stale FRED series · held in dlq.gx_quarantine · retried after suite update" color="#b45309" />
      </div>
...
// DESTINATION / OUTCOME below are vertical-specific for Altavest Capital.
// =============================================================================
function ActivationsPanel() {
  // TRIGGER — the gold-layer condition that fires the sync (vertical-specific)
  const TRIGGER = "A CIK's restricted_list_check flips false → true in gold.dim_holdings — compliance's join-time check against compliance.restricted_tickers just resolved a new legal/MNPI determination, scoped by desk_id (equity, credit, macro).";
  // DESTINATION — the downstream system NewCo Activations pushes into (vertical-specific)
  const DESTINATION = 'Bloomberg AIM · compliance_rule';
  // OUTCOME — the business payoff the SE narrates (vertical-specific)
  const OUTCOME = 'Cuts restricted-list propagation from a 3–6 hour manual export-email-rekey window down to under 2 minutes — closing the gap where a trader could unknowingly execute against a name already under legal restriction, and removing ~6 hours/week of manual reconciliation for compliance ops.';
  ...
  return (
    <section className="mb-8 research-card overflow-hidden" style={cardStyle}>
      ...
      <div className="px-5 py-3 border-t border-[var(--hairline-soft,#e8e4d8)] flex items-center justify-between text-[11px] text-[var(--ink-soft)]" style={{ background: 'var(--paper-deep,#f4efe2)' }}>
        <span>Connections → Destinations → Transformations → <strong style={{ color: '#0e7490' }}>Activations</strong> · one platform, one lineage graph</span>
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
      <div className="research-card p-6 border-l-4" style={{ ...cardStyle, borderLeftColor: '#b91c1c' }}>
       ...
       → Census reverse-ETL → Hightouch → AI vector store
      ...
      <div className="research-card p-6 border-l-4" style={{ ...cardStyle, borderLeftColor: '#b8975c' }}>
       ...
       → NewCo Activations (native) → Bloomberg AIM`}</pre>
        <div className="mt-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="col-span-2 pt-3 mt-1 border-t border-[var(--hairline-soft,#e8e4d8)]">
            <div className="text-[var(--ink-soft)] text-xs">Reverse-ETL hop</div>
            <div className="font-serif text-lg font-semibold" style={{ color: '#0e7490' }}>
